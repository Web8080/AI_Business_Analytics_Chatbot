...
With Quick Wins: Theme Toggle, Export, Chat History, Quick Actions
Run: streamlit run dashboard_enhanced_ui.py
"""
    ...
    if st.session_state.chat_history:
        for i, chat in enumerate(st.session_state.chat_history[-5:]):  # Show last 5
            with st.expander(f"Chat {len(st.session_state.chat_history) - 4 + i}"):
                st.write(f"**Date:** {chat['timestamp']}")
                st.write(f"**File:** {chat['filename']}")
                st.write(f"**Messages:** {len(chat['messages'])}")
                if st.button(f"Load Chat {i+1}", key=f"load_{i}"):
                    st.session_state.messages = chat['messages']
                    st.session_state.uploaded_filename = chat['filename']
                    st.rerun()
    else:
...
with st.expander("❓ Help & Tips"):
    ...
    **💡 How to use this dashboard:**
    ...
    3. **Quick Actions**: Use the sidebar buttons for common queries
